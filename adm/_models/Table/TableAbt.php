...
if (!$Coord) {
    $EmpCoord = " WHERE tb_tickets.status = 'AGUARDANDO'";
} else {
    $EmpCoord = " WHERE tb_empresas.IdEmpresa = '{$_SESSION['userlogin']['IdEmpresaUsuario']}' AND tb_tickets.status = 'AGUARDANDO'";
}
$Read = new Read;
...
INNER JOIN tb_mensagens ON tb_mensagens.idticket = tb_tickets.id {$op}";
    if (!empty($Req['search']['value'])) {
        if (!$Coord) {
            $EmpCoord = " AND tb_tickets.status = 'AGUARDANDO'";
        } else {
            $EmpCoord = " AND tb_empresas.IdEmpresa = '{$_SESSION['userlogin']['IdEmpresaUsuario']}' AND tb_tickets.status = 'AGUARDANDO'";
        }
        $Query .= " WHERE";
