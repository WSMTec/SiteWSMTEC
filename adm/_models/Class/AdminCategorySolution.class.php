...
class AdminCategorySolution {
    ...
    const Entity = 'tb_categories_solution';
    ...
    public function ExeCreate(array $Data) {
        ...
        if (in_array('', $this->Data)):
            $this->Result = false;
            $this->Error = ['<b>Erro ao cadastrar:</b> Para cadastrar uma categoria, preencha todos os campos!', 'blue', 'lnr lnr-warning', 4000];
        else:
            $this->setData();
    ...
    public function ExeUpdate($CategoryId, array $Data) {
        ...
        if (in_array('', $this->Data)):
            $this->Result = false;
            $this->Error = ["<b>Erro ao atualizar:</b> Para atualizar a categoria {$this->Data['category_title']}, preencha todos os campos!", 'blue', 'lnr lnr-warning', 4000];
        else:
            $this->setData();
            $this->setName();
            $this->Update();
    ...
    public function ExeDelete($CategoryId) {
        ...
        if (!$read->getResult()):
            $this->Result = false;
            $this->Error = ['Oppsss, você tentou remover uma categoria que não existe no sistema!', 'blue', 'lnr lnr-warning', 4000];
        else:
            extract($read->getResult()[0]);
            if (!$category_parent && !$this->checkCats()):
                $this->Result = false;
                $this->Error = ["A <b>seção {$category_title}</b> possui categorias cadastradas. Para deletar, antes altere ou remova as categorias filhas!", 'blue', 'lnr lnr-warning', 4000];
            elseif ($category_parent && !$this->checkPosts()):
                $this->Result = false;
                $this->Error = ["A <b>categoria {$category_title}</b> possui soluções cadastradas. Para deletar, antes altere ou remova todas as soluções desta categoria!", 'blue', 'lnr lnr-warning', 4000];
            else:
                $delete = new Delete;
                ...
                $this->Result = true;
                $this->Error = ["A <b>{$tipo} {$category_title}</b> foi removida com sucesso do sistema!", 'green', 'lnr lnr-smile', 4000];
            endif;
        endif;
    }
    ...
    private function checkPosts() {
        $readPosts = new Read;
        $readPosts->ExeRead("tb_solution", "WHERE post_category = :category", "category={$this->CatId}");
    ...
    private function Create() {
        ...
        if ($Create->getResult()):
            ...
            $this->Error = ["<b>Sucesso:</b> A categoria {$this->Data['category_title']} foi cadastrada no sistema!", 'green', 'lnr lnr-smile', 4000];
        endif;
    }
